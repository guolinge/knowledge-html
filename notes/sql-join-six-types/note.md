...
!!不先想清楚执行顺序，这两条规则就记不住，只能硬背。!!

- **第 ① 步·配对** —— `ON` 里的条件在这里生效。它只回答「谁能和谁配成一对」。
  ==配不上 ≠ 这行不要==，只是「没配上」。
- **第 ② 步·`LEFT JOIN` 收尾** —— 左表没配上的行，补上 `NULL` 塞进结果集。
  这一步正是 `LEFT JOIN` 存在的全部理由。
- **第 ③ 步·`WHERE`** —— 对已经成型的结果集逐行筛。
  !!它根本不知道某一行是「配上的」还是「补 NULL 补出来的」，一视同仁。!!

所以：==`ON` 是第 ① 步的筛选器，`WHERE` 是第 ③ 步的筛选器。==

### 盯着「小刚」走一遍

两种写法里，小刚在第 ①② 步的遭遇**完全一样**：

```compare
first: 小刚这一行
head: [写在 ON 里, 写在 WHERE 里]
rows:
  - ① 配对:
      - 条件里带着 status，小刚没订单 → 一个都没配上
      - 条件里只有 id，小刚没订单 → 一个都没配上
  - ② LEFT JOIN 收尾:
      - { text: 保住这行 · o.status 补 NULL, tone: amber }
      - { text: 保住这行 · o.status 补 NULL, tone: amber }
  - ③ WHERE 过滤:
      - 没有 WHERE，原样输出
      - { text: NULL = 'PAID' 结果是 NULL，不是 true → 删掉, tone: red }
  - 最终结果:
      - { text: 3 行 · 小刚还在, tone: green }
      - { text: 2 行 · 小刚消失, tone: red }
```

==小刚在第 ①② 步待遇相同，差别只在第 ③ 步有没有人来筛他。==

（同一张表里的「小明 · 鼠标」也有一条不同的命：它在 `ON` 写法里第 ① 步就因为 `UNPAID` 配不上，
在 `WHERE` 写法里却配上了、直到第 ③ 步才被删。两道筛子都会删行，区别只是**在哪个时刻删**。）

这就是「把 `LEFT JOIN` 退化成 `INNER JOIN`」的意思：
`LEFT JOIN` 的全部价值就是保住这些配不上的行，而 `WHERE` 把它们全删了
—— 剩下的必然两边都有，**那就是 `INNER JOIN` 的结果**。
